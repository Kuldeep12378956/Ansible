...
def lambda_handler(event, context):
    # Print the entire event object to CloudWatch Logs for inspection
    ...
    try:
        ...
        print(f"Error extracting repository or branch name: {e}")
        repository_name = "UnknownRepository"
        branch_name = "UnknownBranch"
    
    commit_id = event['Records'][0]['codecommit']['references'][0]['commit']
    
    # Set the AWS region manually since it's not available directly in the event
    aws_region = "ap-south-1"
    
    # Configure AWS CodeCommit client
    codecommit = boto3.client('codecommit')
    
    # Get commit information
    commit_response = codecommit.get_commit(repositoryName=repository_name, commitId=commit_id)
    commit_message = commit_response['commit']['message']
    author_name = commit_response['commit']['author']['name']
    
    # Get file changes
    try:
        parent_commit = commit_response['commit']['parents'][0]
        diff_resp = codecommit.get_differences(
            repositoryName=repository_name,
            beforeCommitSpecifier=parent_commit,
            afterCommitSpecifier=commit_id
        )
        files = '\n'.join([f'\t{blob["afterBlob"]["path"]}' for blob in diff_resp['differences']])
    except KeyError as e:
        print(f"Error extracting file changes: {e}")
        files = "UnknownFileChanges"
    
    # Configure Slack webhook URL
    slack_webhook_url = os.environ['SLACK_WEBHOOK_URL']
    
    # Prepare Slack notification message with improved formatting
    slack_message = {
        'text': (
            f'*:rocket: AWS CodeCommit Notification* | *{aws_region}* | *Account:* 002807324465 :rocket:\n\n'
            f'    *New Commit in Repo:* `<https://{aws_region}.console.aws.amazon.com/codesuite/codecommit/repositories/{repository_name}/browse|{repository_name}>`\n'
            f'    *Branch:* {branch_name}\n'
            f'    *Modified by:* {author_name}\n'
